...
export class AdminService {
  ...
  async generate() {
    ...
    const result1 = await this.pool.query(`INSERT INTO administrador
                                            ...
                                            VALUES ('adminparking','admin@gmail.com',$1,'admin')`, [hash]);
  ...
  async createUser(data: User) {
    ...
    if (buscarCorreo.rowCount == 0) {
      if (rol == 'socio') {
        ...
        throw new Error('No puede crear este rol')
      }
    }
 ...
 */
  ...
  /**
  * Funcion que permite detalle de vehículos en el parqueadero
  * @returns : QueryResult
  */
  async listarVehiculosParqueaderoDetalle(idParking: number) {
    const query = `SELECT v.nombre, v.placa, v.fechaingreso, p.nombre as parqueadero, s.nombre as socio
                          FROM parqueadero as p
                          JOIN vehiculo as v
                          ON p.id_parking = v.id_parking
    ...
    if (result.rowCount == 0) {
      throw new Error('No hay vehiculos en el parqueadero')
    }
    return result.rows;
  }
  ...
  * @returns : QueryResult
  */
  async listarVehiculosParqueadero(idParking: number) {
    const query = `SELECT v.nombre, v.placa
                          FROM parqueadero as p
                          JOIN vehiculo as v
                          ON p.id_parking = v.id_parking
                          JOIN socio as s
